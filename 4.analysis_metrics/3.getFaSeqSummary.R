...
faSeq_list <- lapply(inbredLines,function(x){
  ...
  outDt <- data.table(
    cbind(
      faId=gsub(" .*","", names(input_seq))
      ,seqLen=width(input_seq)    )
    )
  outDt[,inbred:=x]
  outDt
})
...
faSeqSummary <- faSeq_dt[,list(
  numGenes=.N,
  totAALen=sum(seqLen),
  minAALen=min(seqLen),
  meanAAL=round(mean(seqLen),2),
  medAAL=median(seqLen),
  maxAALen=max(seqLen),
  smallGeneProp=round(sum(seqLen<50)/.N*100,2)
  ),by=inbred]
setorderv(faSeqSummary,"inbred")
cat(kable(faSeqSummary,
          "markdown",
          row.names = F,
          format.args = list(big.mark=","),
          caption = "Fasta Sequence Summary"),
    "\n\n\n",
    file = "tables/README.md",
    sep = "\n",
    append=TRUE)
fwrite(faSeq_dt,
       file = "data/fasta/filt_fasta/faSeqData.tsv",
       sep = "\t",
       row.names = F)
