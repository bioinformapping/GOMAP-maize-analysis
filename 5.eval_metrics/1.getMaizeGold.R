...
maizeCrossRef_genes <- 
  maizeCrossRef_melt[,list(geneModel=splitComma(geneModel)),
                     by=list(inbred,B73v3)]
curatedMaizeAnnot <- read_gaf("data/curated/maize_v3.gold.gaf")


write_gaf(
  curatedMaizeAnnot[grep("^GRM|^AC",db_object_symbol)],
  "data/curated/B73v3.curated.gaf")

annot_merge <- merge.data.table(
  maizeCrossRef_genes,
  curatedMaizeAnnot,
  by.x="B73v3",
  by.y="db_object_symbol")
