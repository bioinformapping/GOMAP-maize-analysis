...
get_alt_id = function(go_obo){
    ...
    alt_conv <- mclapply(names(go_obo$alt_id),function(x){
      out <- unlist(go_obo$alt_id[[x]])
      out2 <- NULL
      if(length(out)>0){
        out2 <- rep(x,length(out))
        names(out2) <- out
      }
      out2
    },mc.cores=4)
    
    return(unlist(alt_conv))
}
...
get_specificity = function(terms,obo_data){
  out <- mclapply(terms,function(x){
      if(x %in% names(obo_data$ancestors)){
        length(obo_data$ancestors[[x]])
      }
      else{
        0
      }
